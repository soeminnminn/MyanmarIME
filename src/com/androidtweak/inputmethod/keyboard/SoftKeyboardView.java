...
package com.androidtweak.inputmethod.keyboard;
...
import com.androidtweak.inputmethod.myanmar.MyanmarIME;
import com.androidtweak.inputmethod.myanmar.MyanmarImeLogger;
import com.androidtweak.inputmethod.myanmar.R;
import com.androidtweak.inputmethod.myanmar.ResearchLogger;
import com.androidtweak.inputmethod.myanmar.StaticInnerHandlerWrapper;
import com.androidtweak.inputmethod.myanmar.StringUtils;
import com.androidtweak.inputmethod.myanmar.SubtypeLocale;
import com.androidtweak.inputmethod.myanmar.Utils;
import com.androidtweak.inputmethod.myanmar.Utils.UsabilityStudyLogUtils;
import com.androidtweak.inputmethod.myanmar.define.ProductionFlag;
import com.androidtweak.inputmethod.keyboard.PointerTracker.DrawingProxy;
import com.androidtweak.inputmethod.keyboard.PointerTracker.TimerProxy;
 ...
 */
public class SoftKeyboardView extends KeyboardView implements PointerTracker.KeyEventHandler,
        SuddenJumpingTouchEventHandler.ProcessMotionEvent {
    private static final String TAG = SoftKeyboardView.class.getSimpleName();

    // TODO: Kill process when the usability study mode was changed.
    private static final boolean ENABLE_USABILITY_STUDY_LOG = MyanmarImeLogger.sUsabilityStudy;
    ...
    private static class KeyTimerHandler extends StaticInnerHandlerWrapper<SoftKeyboardView>
        ...
        public KeyTimerHandler(SoftKeyboardView outerInstance, KeyTimerParams params) {
            super(outerInstance);
            mParams = params;
        }
        ...
        public void handleMessage(Message msg) {
            final SoftKeyboardView keyboardView = getOuterInstance();
    ...
    public static class PointerTrackerParams {
        ...
        public PointerTrackerParams(TypedArray latinKeyboardViewAttr) {
            mSlidingKeyInputEnabled = latinKeyboardViewAttr.getBoolean(
                    R.styleable.SoftKeyboardView_slidingKeyInputEnable, false);
            mTouchNoiseThresholdTime = latinKeyboardViewAttr.getInt(
                    R.styleable.SoftKeyboardView_touchNoiseThresholdTime, 0);
            mTouchNoiseThresholdDistance = latinKeyboardViewAttr.getDimension(
                    R.styleable.SoftKeyboardView_touchNoiseThresholdDistance, 0);
        }
    }
    ...
    static class KeyTimerParams {
        ...
        public KeyTimerParams(TypedArray latinKeyboardViewAttr) {
            mKeyRepeatStartTimeout = latinKeyboardViewAttr.getInt(
                    R.styleable.SoftKeyboardView_keyRepeatStartTimeout, 0);
            mKeyRepeatInterval = latinKeyboardViewAttr.getInt(
                    R.styleable.SoftKeyboardView_keyRepeatInterval, 0);
            mLongPressKeyTimeout = latinKeyboardViewAttr.getInt(
                    R.styleable.SoftKeyboardView_longPressKeyTimeout, 0);
            mLongPressShiftKeyTimeout = latinKeyboardViewAttr.getInt(
                    R.styleable.SoftKeyboardView_longPressShiftKeyTimeout, 0);
            mIgnoreAltCodeKeyTimeout = latinKeyboardViewAttr.getInt(
                    R.styleable.SoftKeyboardView_ignoreAltCodeKeyTimeout, 0);
        }
    }

    public SoftKeyboardView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.softKeyboardViewStyle);
    }
    ...
    public SoftKeyboardView(Context context, AttributeSet attrs, int defStyle) {
        ...
        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.SoftKeyboardView, defStyle, R.style.SoftKeyboardView);
        mAutoCorrectionSpacebarLedEnabled = a.getBoolean(
                R.styleable.SoftKeyboardView_autoCorrectionSpacebarLedEnabled, false);
        mAutoCorrectionSpacebarLedIcon = a.getDrawable(
                R.styleable.SoftKeyboardView_autoCorrectionSpacebarLedIcon);
        mSpacebarTextRatio = a.getFraction(R.styleable.SoftKeyboardView_spacebarTextRatio,
                1000, 1000, 1) / 1000.0f;
        mSpacebarTextColor = a.getColor(R.styleable.SoftKeyboardView_spacebarTextColor, 0);
        mSpacebarTextShadowColor = a.getColor(
                R.styleable.SoftKeyboardView_spacebarTextShadowColor, 0);
        mLanguageOnSpacebarFinalAlpha = a.getInt(
                R.styleable.SoftKeyboardView_languageOnSpacebarFinalAlpha, ALPHA_OPAQUE);
        final int languageOnSpacebarFadeoutAnimatorResId = a.getResourceId(
                R.styleable.SoftKeyboardView_languageOnSpacebarFadeoutAnimator, 0);
        final int altCodeKeyWhileTypingFadeoutAnimatorResId = a.getResourceId(
                R.styleable.SoftKeyboardView_altCodeKeyWhileTypingFadeoutAnimator, 0);
        final int altCodeKeyWhileTypingFadeinAnimatorResId = a.getResourceId(
                R.styleable.SoftKeyboardView_altCodeKeyWhileTypingFadeinAnimator, 0);

        final KeyTimerParams keyTimerParams = new KeyTimerParams(a);
        mPointerTrackerParams = new PointerTrackerParams(a);

        final float keyHysteresisDistance = a.getDimension(
                R.styleable.SoftKeyboardView_keyHysteresisDistance, 0);
        mKeyDetector = new KeyDetector(keyHysteresisDistance);
        mKeyTimerHandler = new KeyTimerHandler(this, keyTimerParams);
        mConfigShowMoreKeysKeyboardAtTouchedPoint = a.getBoolean(
                R.styleable.SoftKeyboardView_showMoreKeysKeyboardAtTouchedPoint, false);
        a.recycle();
    ...
    public void setKeyboard(Keyboard keyboard) {
        ...
        if (ProductionFlag.IS_EXPERIMENTAL) {
            ResearchLogger.softKeyboardView_setKeyboard(keyboard);
        }

        // This always needs to be set since the accessibility state can
        // potentially change without the keyboard being set agacom.
    ...
    protected boolean onLongPress(Key parentKey, PointerTracker tracker) {
        if (ProductionFlag.IS_EXPERIMENTAL) {
            ResearchLogger.softKeyboardView_onLongPress();
        }
        ...
        if (primaryCode == Keyboard.CODE_SPACE || primaryCode == Keyboard.CODE_LANGUAGE_SWITCH) {
            ...
            if (invokeCustomRequest(MyanmarIME.CODE_SHOW_INPUT_METHOD_PICKER)) {
                tracker.onLongPressed();
                invokeReleaseKey(primaryCode);
                return true;
            }
    ...
    public boolean processMotionEvent(MotionEvent me) {
        ...
        if (ProductionFlag.IS_EXPERIMENTAL) {
            ResearchLogger.softKeyboardView_processMotionEvent(me, action, eventTime, index, id,
                    x, y);
        }
        ...
        if (action == MotionEvent.ACTION_MOVE) {
            for (int i = 0; i < pointerCount; i++) {
                ...
                if (ProductionFlag.IS_EXPERIMENTAL) {
                    ResearchLogger.softKeyboardView_processMotionEvent(me, action, eventTime,
                            i, pointerId, px, py);
                }
            }
        } else {
    ...
    private String layoutLanguageOnSpacebar(Paint paint, InputMethodSubtype subtype,
        ...
        text = getShortDisplayName(subtype, getResources());
        if (fitsTextIntoWidth(width, text, paint)) {
            return text;
        }
    ...
    static String getFullDisplayName(InputMethodSubtype subtype, Resources res) {
        ...
        if(locale.getLanguage().equals("my")) {
        	if(locale.getCountry().equals("MM")) {
        		return res.getString(R.string.subtype_my_MM_full);
        	} else if(locale.getCountry().equals("ZG")) {
        		return res.getString(R.string.subtype_my_ZG_full);
        	}
    	}
    ...
    static String getShortDisplayName(InputMethodSubtype subtype, Resources res) {
        if (SubtypeLocale.isNoLanguage(subtype)) {
            return "";
        }
        final Locale locale = SubtypeLocale.getSubtypeLocale(subtype);
        if(locale.getLanguage().equals("my")) {
        	if(locale.getCountry().equals("MM")) {
        		return res.getString(R.string.subtype_my_MM_short);
        	} else if(locale.getCountry().equals("ZG")) {
        		return res.getString(R.string.subtype_my_ZG_short);
        	}
    	}
