...
import static com.androidtweak.inputmethod.keyboard.Keyboard.CODE_OUTPUT_TEXT;
import static com.androidtweak.inputmethod.keyboard.Keyboard.CODE_SHIFT;
import static com.androidtweak.inputmethod.keyboard.Keyboard.CODE_SWITCH_ALPHA_SYMBOL;
import static com.androidtweak.inputmethod.keyboard.Keyboard.CODE_UNSPECIFIED;
import static com.androidtweak.inputmethod.keyboard.internal.KeyboardIconsSet.ICON_UNDEFINED;
...
import com.androidtweak.inputmethod.myanmar.Constants;
import com.androidtweak.inputmethod.myanmar.DeadAccentSequence;
import com.androidtweak.inputmethod.myanmar.R;
import com.androidtweak.inputmethod.myanmar.StringUtils;
import com.androidtweak.inputmethod.myanmar.MyanmarImeUtils;
import com.androidtweak.inputmethod.keyboard.internal.KeySpecParser;
import com.androidtweak.inputmethod.keyboard.internal.KeyboardIconsSet;
import com.androidtweak.inputmethod.keyboard.internal.KeySpecParser.MoreKeySpec;
import com.androidtweak.inputmethod.keyboard.internal.KeyStyles.KeyStyle;
...
public class Key {
    ...
    public Key(Keyboard.Params params, String label, String hintLabel, int iconId,
        ...
        // SMM {
        mFontName = params.mFontName;
        mSizeAdjust = params.mSizeAdjust;
        mDrawFixCode = params.mDrawFixCode;
        mAppendDeadKey = params.mAppendDeadKey;
        mPopupTemplate = params.mMoreKeysTemplate;
        // } SMM
    ...
    public Key(Resources res, Keyboard.Params params, Keyboard.Builder.Row row,
        ...
        mPopupTemplate = keyAttr.getResourceId(R.styleable.Keyboard_Key_popupTemplate, params.mMoreKeysTemplate); // SMM
    ...
    private boolean equals(Key o) {
        ...
        return o.mX == mX
                ...
                && TextUtils.equals(o.mFontName, mFontName); // SMM
    }
    ...
    public boolean isMyChar() {
    	if ((mLabel != null) && (StringUtils.codePointCount(mLabel) > 1)) {
        	return MyanmarImeUtils.isMyChar(mLabel);
    	}
		return MyanmarImeUtils.isMyChar(mCode);
	}
    
    public void fillParams(Keyboard.Params params) {
    	params.mFontName = mFontName;
    	params.mSizeAdjust = mSizeAdjust;
    	params.mDrawFixCode = mDrawFixCode;
    	params.mAppendDeadKey = mAppendDeadKey;
    }
    // }
    
    public Typeface selectTypeface(Context context, Typeface defaultTypeface, boolean bold) {
        // TODO: Handle "bold" here too?
    	if ((mFontName != null) && (!mFontName.equals("default"))) {
    		return Constants.TextUtils.getTypeFace(context, mFontName);
    	} else if ((mLabelFlags & LABEL_FLAGS_FONT_NORMAL) != 0) {
    ...
    public String getDrawableLabel() {
    	if (mLabel == null) return null;
    	
    	String fixLabel = mLabel; 
    	if(fixLabel != null && fixLabel.length() == 1 && mAppendDeadKey) {
    		fixLabel = DeadAccentSequence.getSpacing(fixLabel.charAt(0));
    	}
    	
    	return MyanmarImeUtils.ZawGyiDrawFix(fixLabel, mDrawFixCode).toString();
    }
    
    public String getDrawableHintLabel() {
    	if (mHintLabel == null) return null;
    	
    	String fixLabel = mHintLabel; 
    	if(fixLabel != null && fixLabel.length() == 1 && mAppendDeadKey) {
    		fixLabel = DeadAccentSequence.getSpacing(fixLabel.charAt(0));
    	}
    ...
    public String getPreviewLabel() {
    	String fixLabel = isShiftedLetterActivated() ? mHintLabel : mLabel;
    	if(fixLabel != null && fixLabel.length() == 1 && mAppendDeadKey) {
    		fixLabel = DeadAccentSequence.getSpacing(fixLabel.charAt(0));
    	}
