...
public class KeyboardState {
    ...
    // SMM {
    private void toggleShiftInAlphabet() {
    	if (!mIsAlphabetMode) return;
    ...
    public void onPressKey(int code, boolean isSinglePointer, int autoCaps, boolean shiftCap) { // SMM
        if (DEBUG_EVENT) {
        ...
        if (code == Keyboard.CODE_SHIFT) {
            ...
            if (!isSinglePointer && mIsAlphabetMode && autoCaps != TextUtils.CAP_MODE_CHARACTERS) {
                ...
                if (needsToResetAutoCaps && shiftCap) { // SMM
                    mSwitchActions.setAlphabetKeyboard();
                }
            }
        }
    ...
    public void onUpdateShiftState(int autoCaps, boolean shiftCap) {
        if (DEBUG_EVENT) {
            Log.d(TAG, "onUpdateShiftState: autoCaps=" + autoCaps + " " + this);
        }
    	
        updateAlphabetShiftState(autoCaps, shiftCap);
    }

    private void updateAlphabetShiftState(int autoCaps, boolean shiftCap) { // SMM
        if (!mIsAlphabetMode) return;
        if(!shiftCap) return; // SMM
    ...
    private void onPressShift(boolean shiftCap) { // SMM
        mLongPressShiftLockFired = false;
        if (mIsAlphabetMode) {
        	if(shiftCap) {
    ...
    public void onCodeInput(int code, boolean isSinglePointer, int autoCaps, boolean shiftCap) { // SMM
        if (DEBUG_EVENT) {
        ...
        if (Keyboard.isLetterCode(code)) {
            updateAlphabetShiftState(autoCaps, shiftCap); // SMM
        }
    }
