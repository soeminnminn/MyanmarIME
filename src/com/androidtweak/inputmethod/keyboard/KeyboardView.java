...
public class KeyboardView extends View implements PointerTracker.DrawingProxy {
    ...
    public void setKeyboard(Keyboard keyboard) {
        mKeyboard = keyboard;
        MyanmarImeLogger.onSetKeyboard(keyboard);
        requestLayout();
        invalidateAllKeys();
    ...
    protected void onDrawKeyTopVisuals(Key key, Canvas canvas, Paint paint, KeyDrawParams params) {
        ...
        if (key.mLabel != null) {
            final String label = key.getDrawableLabel();
            // For characters, use large font. For labels like "Done", use smaller font.
            paint.setTypeface(key.selectTypeface(getContext(), params.mKeyTextStyle, false));
        ...
        if (key.mHintLabel != null) {
            final String hint = key.getDrawableHintLabel();
            final int hintColor;
            final int hintSize;
            if (key.hasHintLabel()) {
    ...
    public void showKeyPreview(PointerTracker tracker) {
        ...
        if (label != null) {
            ...
            if (StringUtils.codePointCount(label) > 1) {
                ...
                previewText.setTypeface(key.selectTypeface(getContext(), Typeface.DEFAULT_BOLD, true));
            } else {
                previewText.setTextSize(TypedValue.COMPLEX_UNIT_PX, params.mPreviewTextSize);
                previewText.setTypeface(key.selectTypeface(getContext(), params.mKeyTextStyle, false));
            }
            previewText.setText(key.getPreviewLabel());
        } else {
            previewText.setCompoundDrawables(null, null, null, key.getPreviewIcon(mKeyboard.mIconsSet));
        ...
        if (previewX < 0) {
            ...
            if (params.mPreviewLeftBackground != null) {
                previewText.setBackground(params.mPreviewLeftBackground);
            }
        } else if (previewX > getWidth() - previewWidth) {
            previewX = getWidth() - previewWidth;
            if (params.mPreviewRightBackground != null) {
                previewText.setBackground(params.mPreviewRightBackground);
            }
        }
