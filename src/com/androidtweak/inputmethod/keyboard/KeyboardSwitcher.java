...
import com.androidtweak.inputmethod.myanmar.DebugSettings;
import com.androidtweak.inputmethod.myanmar.ImfUtils;
import com.androidtweak.inputmethod.myanmar.InputView;
...
import com.androidtweak.inputmethod.myanmar.SettingsValues;
import com.androidtweak.inputmethod.myanmar.SubtypeSwitcher;
import com.androidtweak.inputmethod.myanmar.Utils;
import com.androidtweak.inputmethod.keyboard.KeyboardLayoutSet.KeyboardLayoutSetException;
...
public class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    public static void init(MyanmarIME myanmarIme, SharedPreferences prefs) {
        sInstance.initInternal(myanmarIme, prefs);
    }

    private void initInternal(MyanmarIME myanmarIme, SharedPreferences prefs) {
        mIME = myanmarIme;
        mResources = myanmarIme.getResources();
        mPrefs = prefs;
        mSubtypeSwitcher = SubtypeSwitcher.getInstance();
        mState = new KeyboardState(this);
        setContextThemeWrapper(myanmarIme, getKeyboardTheme(myanmarIme, prefs));
    ...
    public void loadKeyboard(EditorInfo editorInfo, SettingsValues settingsValues) {
        ...
        try {
            ...
            MyanmarImeLogger.logOnException(e.mKeyboardId.toString(), e.getCause());
            return;
        }
    }
    ...
    private void setKeyboard(final Keyboard keyboard) {
        ...
        mKeyboardView.startDisplayLanguageOnSpacebar(subtypeChanged, needsToDisplayLanguage,
                ImfUtils.hasMultipleEnabledIMEsOrSubtypes(mIME, true));
    }
    ...
    public void updateShiftState() {
        mState.onUpdateShiftState(mIME.getCurrentAutoCapsState(), isShiftCapEnable()); // SMM
    }

    public void onPressKey(int code) {
        if (isVibrateAndSoundFeedbackRequired()) {
            mIME.hapticAndAudioFeedback(code);
        }
        mState.onPressKey(code, isSinglePointer(), mIME.getCurrentAutoCapsState(), isShiftCapEnable());
    }
    ...
    public void startDoubleTapTimer() {
        final SoftKeyboardView keyboardView = getKeyboardView();
    ...
    // SMM {
    private boolean isShiftCapEnable() {
    	final Keyboard keyboard = getKeyboard();
    	return keyboard != null && keyboard.mShiftCapEnable;
    }
    
    public InputMethodSubtype getCurrentSubType() {
    	final Keyboard keyboard = getKeyboard();
    	return keyboard.mId.mSubtype;
    }
    // } SMM
    ...
    public void onCodeInput(int code) {
        mState.onCodeInput(code, isSinglePointer(), mIME.getCurrentAutoCapsState(), isShiftCapEnable()); // SMM
    }
    ...
    public View onCreateInputView() {
        ...
        for (int i = 0; i < Utils.GCUtils.GC_TRY_LOOP_MAX && tryGC; ++i) {
            try {
                setContextThemeWrapper(mIME, mKeyboardTheme);
        ...
        mKeyboardView = (SoftKeyboardView) mCurrentInputView.findViewById(R.id.keyboard_view);
        mKeyboardView.setKeyboardActionListener(mIME);
