...
public class WordComposer {
    ...
    private CharSequence mFixedWord;
    private boolean mUsedFixedWord;
    ...
    private void add(int primaryCode, int keyX, int keyY) {
        ...
        fixedCharIndex(primaryCode); // SMM
    ...
    // SMM {
    private void fixedCharIndex(int primaryCode) {
    	if(mLocale != null) {
    		if (mLocale.getLanguage().equals("my")) {
    			if (mLocale.getCountry().equals("MM")) {
        			// TODO: Fix up for My_MM
        		}
    		}
    	}
    }
    
    public CharSequence setFixedWord(final CharSequence word) {
    	mFixedWord = word;
    	return mFixedWord;
    }
    
    public String getFixedWord() {
    	if ((mFixedWord == null) || (mFixedWord.length() == 0)) {
    		return mTypedWord.toString();
    	}
    	return mFixedWord.toString();
    }
    
    public void setUsedFixedWord(boolean used) {
    	mUsedFixedWord = used;
    }
    
    public boolean getUsedFixedWord() {
    	return mUsedFixedWord;
    }
    // } SMM
    ...
    public void deleteLast() {
        ...
        if (size > 0) {
            ...
            // SMM {
            if(mUsedFixedWord && mFixedWord != null) {
            	final StringBuilder fixedBuilder = new StringBuilder();
            	fixedBuilder.append(mFixedWord);
            	final int fixedLength = fixedBuilder.length();
            	
            	final int fixLastChar = fixedBuilder.codePointBefore(fixedLength);
                if (Character.isSupplementaryCodePoint(fixLastChar)) {
                	fixedBuilder.delete(fixedLength - 2, fixedLength);
                } else {
                	fixedBuilder.deleteCharAt(fixedLength - 1);
                }
            	
            	mFixedWord = fixedBuilder.toString();
            }
            // } SMM
        }
    ...
    public CharSequence getAutoCorrectionOrNull() {
    	// SMM {
    	if(mAutoCorrection != null 
    			&& android.text.TextUtils.equals(mAutoCorrection.toString(), mTypedWord.toString()) 
    			&& mFixedWord != null) {
			return mFixedWord;
    	}
    	// } SMM
    ...
    public LastComposedWord commitWord(final int type, final String committedWord,
        ...
        mXCoordinates = new int[N];
        mYCoordinates = new int[N];
        final String typedWord = mUsedFixedWord && mFixedWord != null ? mFixedWord.toString() : mTypedWord.toString(); // SMM
        final LastComposedWord lastComposedWord = new LastComposedWord(primaryKeyCodes,
                xCoordinates, yCoordinates, typedWord, committedWord, separatorCode,
                prevWord);
