...
import com.androidtweak.inputmethod.keyboard.Key;
import com.androidtweak.inputmethod.keyboard.KeyDetector;
import com.androidtweak.inputmethod.keyboard.Keyboard;
import com.androidtweak.inputmethod.keyboard.internal.KeyboardState;
...
/**
 * Logs the use of the MyanmarIME keyboard.
 *
...
public class ResearchLogger implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static class UnsLogGroup {
        ...
        private static final boolean IME_COMMITCURRENTAUTOCORRECTION_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_COMMITTEXT_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_DELETESURROUNDINGTEXT_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_DOUBLESPACEAUTOPERIOD_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_ONDISPLAYCOMPLETIONS_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_ONSTARTINPUTVIEWINTERNAL_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_ONUPDATESELECTION_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_PERFORMEDITORACTION_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_PICKAPPLICATIONSPECIFIEDCOMPLETION_ENABLED
                = DEFAULT_ENABLED;
        private static final boolean IME_PICKPUNCTUATIONSUGGESTION_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_PICKSUGGESTIONMANUALLY_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_REVERTCOMMIT_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_REVERTDOUBLESPACEWHILEINBATCHEDIT_ENABLED
                = DEFAULT_ENABLED;
        private static final boolean IME_REVERTSWAPPUNCTUATION_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_SENDKEYCODEPOINT_ENABLED = DEFAULT_ENABLED;
        private static final boolean IME_SWAPSWAPPERANDSPACEWHILEINBATCHEDIT_ENABLED
                = DEFAULT_ENABLED;
        private static final boolean IME_SWITCHTOKEYBOARDVIEW_ENABLED = DEFAULT_ENABLED;
    ...
    public static void ime_commitCurrentAutoCorrection(final String typedWord,
            final String autoCorrection) {
        if (UnsLogGroup.IME_COMMITCURRENTAUTOCORRECTION_ENABLED) {
    ...
    public static void ime_commitText(final CharSequence typedWord) {
        if (UnsLogGroup.IME_COMMITTEXT_ENABLED) {
            logUnstructured("MyanmarIME_commitText", typedWord.toString());
        }
    }

    public static void ime_deleteSurroundingText(final int length) {
        if (UnsLogGroup.IME_DELETESURROUNDINGTEXT_ENABLED) {
            logUnstructured("MyanmarIME_deleteSurroundingText", String.valueOf(length));
        }
    }

    public static void ime_doubleSpaceAutoPeriod() {
        if (UnsLogGroup.IME_DOUBLESPACEAUTOPERIOD_ENABLED) {
            logUnstructured("MyanmarIME_doubleSpaceAutoPeriod", "");
        }
    }

    public static void ime_onDisplayCompletions(
            final CompletionInfo[] applicationSpecifiedCompletions) {
        if (UnsLogGroup.IME_ONDISPLAYCOMPLETIONS_ENABLED) {
            final StringBuilder builder = new StringBuilder();
            ...
            logUnstructured("MyanmarIME_onDisplayCompletions", builder.toString());
        }
    }

    public static void ime_onStartInputViewInternal(final EditorInfo editorInfo,
            final SharedPreferences prefs) {
        if (UnsLogGroup.IME_ONSTARTINPUTVIEWINTERNAL_ENABLED) {
            final StringBuilder builder = new StringBuilder();
            ...
            logUnstructured("MyanmarIME_onStartInputViewInternal", builder.toString());
        }
    }

    public static void ime_onUpdateSelection(final int lastSelectionStart,
            final int lastSelectionEnd, final int oldSelStart, final int oldSelEnd,
            final int newSelStart, final int newSelEnd, final int composingSpanStart,
            final int composingSpanEnd) {
        if (UnsLogGroup.IME_ONUPDATESELECTION_ENABLED) {
            final String s = "onUpdateSelection: oss=" + oldSelStart
                    ...
                    + ", ce=" + composingSpanEnd;
            logUnstructured("MyanmarIME_onUpdateSelection", s);
        }
    }

    public static void ime_performEditorAction(final int imeActionNext) {
        if (UnsLogGroup.IME_PERFORMEDITORACTION_ENABLED) {
            logUnstructured("MyanmarIME_performEditorAction", String.valueOf(imeActionNext));
        }
    }

    public static void ime_pickApplicationSpecifiedCompletion(final int index,
            final CharSequence text, int x, int y) {
        if (UnsLogGroup.IME_PICKAPPLICATIONSPECIFIEDCOMPLETION_ENABLED) {
            final String s = String.valueOf(index) + '\t' + text + '\t' + x + '\t' + y;
            logUnstructured("MyanmarIME_pickApplicationSpecifiedCompletion", s);
        }
    }

    public static void ime_pickSuggestionManually(final String replacedWord,
            final int index, CharSequence suggestion, int x, int y) {
        if (UnsLogGroup.IME_PICKSUGGESTIONMANUALLY_ENABLED) {
            final String s = String.valueOf(index) + '\t' + suggestion + '\t' + x + '\t' + y;
            logUnstructured("MyanmarIME_pickSuggestionManually", s);
        }
    }

    public static void ime_punctuationSuggestion(final int index,
            final CharSequence suggestion, int x, int y) {
        if (UnsLogGroup.IME_PICKPUNCTUATIONSUGGESTION_ENABLED) {
            final String s = String.valueOf(index) + '\t' + suggestion + '\t' + x + '\t' + y;
            logUnstructured("MyanmarIME_pickPunctuationSuggestion", s);
        }
    }

    public static void ime_revertDoubleSpaceWhileInBatchEdit() {
        if (UnsLogGroup.IME_REVERTDOUBLESPACEWHILEINBATCHEDIT_ENABLED) {
            logUnstructured("MyanmarIME_revertDoubleSpaceWhileInBatchEdit", "");
        }
    }

    public static void ime_revertSwapPunctuation() {
        if (UnsLogGroup.IME_REVERTSWAPPUNCTUATION_ENABLED) {
            logUnstructured("MyanmarIME_revertSwapPunctuation", "");
        }
    }

    public static void ime_sendKeyCodePoint(final int code) {
        if (UnsLogGroup.IME_SENDKEYCODEPOINT_ENABLED) {
            logUnstructured("MyanmarIME_sendKeyCodePoint", String.valueOf(code));
        }
    }

    public static void ime_swapSwapperAndSpaceWhileInBatchEdit() {
        if (UnsLogGroup.IME_SWAPSWAPPERANDSPACEWHILEINBATCHEDIT_ENABLED) {
            logUnstructured("ime_swapSwapperAndSpaceWhileInBatchEdit", "");
        }
    }

    public static void ime_switchToKeyboardView() {
        if (UnsLogGroup.IME_SWITCHTOKEYBOARDVIEW_ENABLED) {
            final String s = "Switch to keyboard view.";
            logUnstructured("MyanmarIME_switchToKeyboardView", s);
        }
    }
    ...
    public static void softKeyboardView_onLongPress() {
        if (UnsLogGroup.LATINKEYBOARDVIEW_ONLONGPRESS_ENABLED) {
            ...
            logUnstructured("SoftKeyboardView_onLongPress", s);
        }
    }

    public static void softKeyboardView_processMotionEvent(MotionEvent me, int action,
    ...
    public static void softKeyboardView_setKeyboard(final Keyboard keyboard) {
        if (UnsLogGroup.LATINKEYBOARDVIEW_SETKEYBOARD_ENABLED) {
            ...
            builder.append("]");
            logUnstructured("SoftKeyboardView_setKeyboard", builder.toString());
        }
    }

    public static void ime_revertCommit(final String originallyTypedWord) {
        if (UnsLogGroup.IME_REVERTCOMMIT_ENABLED) {
            logUnstructured("MyanmarIME_revertCommit", originallyTypedWord);
        }
    }
