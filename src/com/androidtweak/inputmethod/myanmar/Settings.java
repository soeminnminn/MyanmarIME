...
import com.androidtweak.inputmethod.common.InputMethodSettingsFragment;
...
public class Settings extends InputMethodSettingsFragment
    ...
    public static final String PREF_USED_UNICODE = "used_unicode"; // SMM
    public static final String PREF_JELLYBEAN_FIX = "jellybean_fix"; // SMM
    public static final String PREF_TEST_AREA = "test_area"; // SMM
    public static final String PREF_ABOUT = "about"; // SMM
    ...
    public void onCreate(Bundle icicle) {
        ...
        // SMM {
        final boolean showTestAreaOption = res.getBoolean(R.bool.config_enable_show_test_area);
        if(showTestAreaOption) {
        	final Preference testArea = (Preference)findPreference(PREF_TEST_AREA);
        	testArea.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            	@Override
        	...
        	advancedSettings.removePreference(findPreference(PREF_TEST_AREA));
        }
        // } SMM
        ...
        // SMM {
        final boolean showUsedUnicodeOption = res.getBoolean(R.bool.config_enable_used_unicode);
        if(showUsedUnicodeOption) {
        	generalSettings.removePreference(findPreference(PREF_USED_UNICODE));
        } 
        
        final boolean showJellybeanFixOption = res.getBoolean(R.bool.config_enable_jellybean_fix);
        if(!showJellybeanFixOption) {
        	generalSettings.removePreference(findPreference(PREF_JELLYBEAN_FIX));
        }
        
        final Preference aboutPrefs = (Preference)findPreference(PREF_ABOUT);
        try {
			PackageInfo pInfo = context.getPackageManager().getPackageInfo(context.getPackageName(), 0);
			aboutPrefs.setSummary(getText(R.string.version_name) + " " + pInfo.versionName);
    ...
    private void showKeyboardTestDialog() {
		...
		dialog.setTitle(R.string.aosp_android_keyboard_ime_name);
		
		final LayoutInflater inflater = LayoutInflater.from(context);
		final RelativeLayout testArea = (RelativeLayout)inflater.inflate(R.layout.test_area, null);
		
		final EditText editTextArea = (EditText)testArea.findViewById(R.id.editTextArea);
		editTextArea.setTypeface(Constants.TextUtils.getZawGyiTypeFace(context));
		
		final Switch swichFont = (Switch)testArea.findViewById(R.id.switchFont);
		swichFont.setOnCheckedChangeListener(new Switch.OnCheckedChangeListener() {
			
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if(isChecked) {
					editTextArea.setTypeface(Constants.TextUtils.getMyanmarTypeFace(context));
				} else {
					editTextArea.setTypeface(Constants.TextUtils.getZawGyiTypeFace(context));			
				}
			}
